...
func add_action(action_str, key_str, effect_ticks=0, effect_str=null):
	if effect_str != null:
		print("InfoPanel.add_action(): Adding Action to InfoPanel: " + action_str + ", key: " + key_str + ", effect: " + effect_str + ", ticks: " + str(effect_ticks))
	else:
		print("InfoPanel.add_action(): Adding Action to InfoPanel: " + action_str + ", key: " + key_str + ", ticks: " + str(effect_ticks))
...
func get_action(input: String):
	...
	print("ERROR (InfoPanel.get_action()): Effect Dict does not have action: " + input)
	return null

func get_ticks(input: String):
	if effect_dict.has(input):
		return effect_dict[input][1]
	print("ERROR (InfoPanel.get_ticks()): Effect Dict does not have action: " + input)
	return -1
